...
def fast_decompress_dxt1(compressed_data, width, height):
    ...
    try:
        ...
        if isinstance(compressed_data, str):
            input_buffer = ctypes.create_string_buffer(compressed_data, len(compressed_data))
        else:
...
def tex_save(image, drawable, filename, raw_filename):
    ...
    try:
        ...
        if width % 4 != 0 or height % 4 != 0:
            error_msg = "Image dimensions must be divisible by 4 for TEX format.\n\n"
            error_msg += "Current size: {}x{}\n".format(width, height)
            error_msg += "Width: {} (needs to be {})\n".format(width, ((width + 3) // 4) * 4)
            error_msg += "Height: {} (needs to be {})\n\n".format(height, ((height + 3) // 4) * 4)
        ...
        if not use_dds_method:
            # Get pixel data (OPTIMIZED - direct memory access)
            pr = drawable.get_pixel_rgn(0, 0, width, height, False, False)
            pixel_data = pr[0:width, 0:height]
            
            # ULTRA-FAST conversion using array module (10-100x faster than list comprehension)
            import array
            if isinstance(pixel_data, str):
                # Python 2 style string - use array for fast conversion
                rgba_data = array.array('B', pixel_data).tostring()
            else:
                # Already bytes
                rgba_data = bytes(pixel_data)
            
            # Ensure we have the right amount of data
            expected_size = width * height * 4
            if len(rgba_data) < expected_size:
                # Pad with zeros if needed
                rgba_data += b'\x00' * (expected_size - len(rgba_data))
