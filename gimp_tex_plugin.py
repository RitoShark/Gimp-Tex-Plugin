...
def tex_save(image, drawable, filename, raw_filename):
    """Save a GIMP image as a .tex file"""
    try:
        ...
        if len(pixel_data) >= width * height * bytes_per_pixel:
            for i in range(width * height * bytes_per_pixel):
                rgba_data[i] = ord(pixel_data[i]) if isinstance(pixel_data[i], str) else pixel_data[i]
        ...
        tex.mipmaps = False
        tex.data = [compress_rgba_to_dxt5(str(rgba_data), width, height)]
        
        # Write file
        tex.write(filename)
        
        sys.stderr.write("Save successful!\n")
        sys.stderr.flush()
